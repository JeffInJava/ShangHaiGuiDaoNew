...
import com.ineasnet.shanghaiguidao2.adapters.IndexListViewAdapter;
...
public class IndexNewFragment extends MyBaseFragment implements View.OnClickListener,CommonProgressDialog.OnOkAndCancelListener,AbPullToRefreshView.OnHeaderRefreshListener,AbPullToRefreshView.OnFooterLoadListener{
    private View v;
     ...
     */
    private  Map<String, TextView> map_tab = new LinkedHashMap<String, TextView>();
    /**
     * 存放导航条的textview key 父节点名称
     */
    private LinkedHashMap<String, TypeVo> titleMap = new LinkedHashMap<String, TypeVo>();
    private List<TextView> titleList = new ArrayList<>();

    /**
     * 存放tab的顺序 用于删除
     */
    private Map<String, Integer> map_tab_order = new LinkedHashMap<String, Integer>();
    ...
    private MyListView lv_index_new;
...
//        activity.registerReceiver(mReceiver, filter);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // TODO Auto-generated method stub
        v = inflater.inflate(R.layout.fragment_indexnew, null);
        ...
        lv_index_new = (MyListView) v.findViewById(R.id.lv_index_new);
...
//        }
        ...
        gridView_sec.setOnItemClickListener(new AdapterView.OnItemClickListener() {
...
//        userList = gson.fromJson(usersJson, new TypeToken<List<User>>(){}.getType());
        try {
            userList = db.selector(User.class).where("userId","=",userId).findAll();
        } catch (DbException e) {
            e.printStackTrace();
        }
     ...
     */
    AdapterView.OnItemClickListener indexListViewClick = new AdapterView.OnItemClickListener() {
...
//                    initDatas(result);
    ...
    private void initDatas() {
        ...
        if (isFromPro) {
            ...
            listViewAdapter = new IndexListViewAdapter(mContext,db ,TypeQueryUtil.QueryByTypeFirst(mContext,db,"29"));
            lv_index_new.setAdapter(listViewAdapter);
            gridAdapter = new MyGridAdapter(mContext, typeMap.get(1));
            gridView.setAdapter(gridAdapter);
            ...
            listViewAdapter.setOnGridItemClickListener(new OnGridItemClickListener() {
                @Override
                public void getView(ViewGroup viewGroup, View view, int position, TypeVo typeVo) {
                    isClickTab = false;
                    folderName = typeVo.getName();
                    titleVo = typeVo;
                    titleMap.put(typeVo.getName(),typeVo);
                    dataCount+=1;
                    if (!isFromPro) {
                        showChild(typeVo);
//                        showSecGridView(typeVo);
                    }
                }
            });
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_guide_index:
                ...
                for (int i=0;i<titleList.size();i++){
                    layout_guide.removeView(titleList.get(i));
                    map_tab.remove(titleList.get(i));
                }
...
//                startSearchTypePopupWindow();
    ...
    private void Query(){
        ...
        switch (typeCount){
            case 0:
                if (QueryUtil.QueryByPDFName(getActivity(),db,search_key,page) == null||QueryUtil.QueryByPDFName(getActivity(),db,search_key,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFName(getActivity(),db,search_key,page);
                }
                break;
            case 1:
                if (QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,page) == null||QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,page);
                }
                break;
            case 2:
                if (QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,page) == null||QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,page);
                }
                break;
        }
    ...
    private void Query(String type){
        ...
        switch (typeCount){
            case 0:
                if (QueryUtil.QueryByPDFName(getActivity(),db,search_key,type,page) == null||QueryUtil.QueryByPDFName(getActivity(),db,search_key,type,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFName(getActivity(),db,search_key,type,page);
                }
                break;
            case 1:
                if (QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,type,page) == null||QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,type,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFDocNum(getActivity(),db,search_key,type,page);
                }
                break;
            case 2:
                if (QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,type,page) == null||QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,type,page).size()<1){
                    showList.clear();
                }else{
                    resultTree = QueryUtil.QueryByPDFRuleNum(getActivity(),db,search_key,type,page);
                }
                break;
        }
        showChildList(resultTree);
    ...
    private void showSecGridView(TypeVo vo) {
        ...
        addGuideTab(vo.getName(),vo.getId());
    ...
    private void showChildList(List<TreeVo> child) {
        ...
        List<TreeVo> clearList = new ArrayList<>();
        for (int i=0;i<showList.size();i++){
            if (!showList.get(i).getName().contains("附件")){
                clearList.add(showList.get(i));
            }
        }
        showList.clear();
        showList = clearList;
...
//            }
    ...
    private String getTabName(){
        String tabName = "";
        String tabOne = "";
        for (int i=0;i<titleList.size();i++) {// 遍历guidetab
            // 的map
            // 取出所有TextView
            // 将不是当前节点的tab字体颜色改成黑色
            tabName +=titleList.get(i).getText().toString().trim();
        }
        if (tabName!=null&&!tabName.equals("")){
            String[] result = new String[]{};
            result = tabName.split("/");
            String tabNameResult = "";
            TypeVo typeOne = TypeQueryUtil.QueryByTypeID(mContext,db, (String) titleList.get(0).getTag());
            for (int i=0;i<result.length;i++){
                tabNameResult += result[i].trim()+"/";
            }
            tabNameResult = tabNameResult.substring(0,tabNameResult.length()-1);
            try {
                TypeVo typeLast = db.selector(TypeVo.class).where("id","=",typeOne.getParentId()).findFirst();
                if (typeLast!=null){
                    String titleOne;
                    if (typeLast.getName().equals("企业标准")){
                        titleOne = "运管中心规章制度体系/";
                    }else{
                        titleOne = typeLast.getName()+"/";
                    }

                    tabNameResult = titleOne+tabNameResult;
                }
            } catch (DbException e) {
                e.printStackTrace();
            }
            return tabNameResult;
        }else{
            return "";
        }
     ...
     */
    private void addGuideTab(String tabName,String id) {
        ...
        tv_tab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                // TODO Auto-generated method stub
                TextView tvNew = (TextView) v;
                isSearched = false;
                tvNew.setTextColor(getResources().getColor(
                        R.color.color_text_guide_selected));
                int num = titleList.indexOf(tvNew);
                if (num<titleList.size()-1){
                    for (int i=num+1;i<titleList.size();i++){
                            TextView tv = titleList.get(i);
                            layout_guide.removeView(tv);
                            map_tab.remove(tv);
                            titleMap.remove(tv.getText().toString());
                            titleList.remove(i);
                    }
                }
...
//                        iterator.remove();
//                        map_tab.remove(tabName_temp);
//                        titleMap.remove(tabName_temp);
//                    }
//                }
                String id = (String) v.getTag();
                TypeVo vo = TypeQueryUtil.QueryByTypeID(mContext,db,id);
                titleVo = vo;
                List typeVoListNew = null;
                if (TypeQueryUtil.QueryByType(mContext,db,vo.getId())!=null&&TypeQueryUtil.QueryByType(mContext,db,vo.getId()).size()>0){
                    typeVoListNew = TypeQueryUtil.QueryByType(mContext,db,vo.getId());
...
//        typeList.add(checkBean4);
    ...
    private void initType(){
        ...
        list_user2.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                typeCount = i;
...
//            list_user.setAdapter(userCheckAdapter);
    ...
    private void getBg(boolean flag){
        if (flag){
            ll_main_bg.setBackground(MyApplication.getInstance().getResources().getDrawable(R.drawable.index_bg_new));
        }else{
    ...
    public void getLastList(){
        ...
        TextView lastTV = titleList.get(titleList.size()-1);
        layout_guide.removeView(lastTV);
        map_tab.remove(lastTV);
        titleList.remove(lastTV);
        if (titleList.size()>0){
            TextView secondTV = titleList.get(titleList.size()-1);
            secondTV.setTextColor(getResources().getColor(
                    R.color.color_text_guide_selected));
            for (int i=0;i<titleList.size()-1;i++){
                TextView tv = titleList.get(i);
            ...
            TypeVo vo = TypeQueryUtil.QueryByTypeID(mContext,db, (String) titleList.get(titleList.size()-1).getTag());
            if (vo !=null){
                List typeVoListNew = null;
                typeVoListNew = TypeQueryUtil.QueryByType(mContext,db,vo.getId());
                gridAdapter_sec = new MyGridAdapter(mContext, typeVoListNew);
